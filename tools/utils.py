...
def image_of_class(y, imagenet_path=None, metadata_path=None):
    """
    ...
    return get_image(im_indices[y], imagenet_path, metadata_path)[0].copy()
...
def get_image(index, imagenet_path=None, metadata_path=None):
    header_row, rows = default_flist_reader(metadata_path)		
    def get(index):
        row_idx_image_id = header_row.index('ImageId')
        row_idx_true_label = header_row.index('TrueLabel')
        row = rows[index]
        filepath = os.path.join(imagenet_path, row[row_idx_image_id] + '.png')
        x = load_image(filepath)
        y = int(row[row_idx_true_label])
        return x, y, filepath
    return get(index)
